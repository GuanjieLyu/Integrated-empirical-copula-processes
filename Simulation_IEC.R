############################################
### Integrated Empirical Copula Test for Independence
### Compares ECT vs IECT via Monte Carlo simulation
############################################
rm(list = ls())
...
##============================================================
## Parallel mclapply with progress and elapsed time
##============================================================
...
num_simulations <- 5000   # for critical values and empirical level
num_power       <- 1000   # for power
ncores          <- detectCores() - 4

##============================================================
## DGP functions — all return pseudo-observations in [0,1]^2
##============================================================

## DGP functions return raw data; pobs() is applied at test-statistic stage
...
##============================================================
## Plot samples (n=1000 for illustration)
##============================================================
plot_titles <- c("(a) Four Clouds", "(b) W-shape", "(c) Diamond",
                 "(d) Parabola", "(e) Two Parabolas", "(f) Circle",
                 "(g) Hyperplane", "(h) Student-t copula")

par(mfrow = c(2, 4), mar = c(2, 2, 2, 2))
for (i in 1:8) {
  samp <- dgp_funs[[i]](1000)
  plot(samp[, 1], samp[, 2], xlab = "", ylab = "", axes = FALSE,
       main = plot_titles[i], col = "steelblue", pch = 16, cex = 0.4)
}
par(mfrow = c(1, 1))
...
compute_stats <- function(uvdat) {
  ...
  ## --- S2, S3, S4 via O(n^2) sweep (for IECT) ---
  ord    <- order(U)
  Ubar_s <- Ubar[ord]
  Vbar_s <- Vbar[ord]
  ...
  Rnbar <- n * ((S4 + 2*S3 + S2) / (4*n^4) - (T2 + T1) / (18*n^2) + 1/100)

  ## --- ECT statistic Bn (Genest, Quessy & Remillard 2006, Section 1) ---
  ## Input uvdat contains pobs() output: U = rank/(n+1), so rank = round(U*(n+1))
  R  <- round(U * (n + 1))
  S  <- round(V * (n + 1))
  DR <- outer(R, R, function(s, t)
    (2*n+1)/(6*n) + s*(s-1)/(2*n*(n+1)) + t*(t-1)/(2*n*(n+1)) - pmax(s,t)/(n+1))
...
all_results <- list()
cv_list     <- list()   # store critical values for reuse
...
for (n in n_grid) {
  ...
  h0_stats <- mclapply_progress(seq_len(num_simulations),
  ...
  level_stats <- mclapply_progress(seq_len(num_simulations),
                                   function(i) compute_stats(pobs(cbind(runif(n), runif(n)))),
  ...
  for (i in seq_along(dgp_funs)) {
    pow_stats <- mclapply_progress(seq_len(num_power),
...
##============================================================
## Power curve: Joe copula with parameter theta deviating from
## independence (theta = 1) toward stronger positive dependence
##============================================================
## Joe copula: C(u,v) = 1 - [(1-u)^theta + (1-v)^theta - (1-u)^theta*(1-v)^theta]^(1/theta)
## theta = 1 → independence; theta > 1 → positive dependence
...
for (n in n_grid) {
  ...
  ## Fresh critical values under H0 for this section
  cat(sprintf("  Critical values (%d simulations)...\n", num_simulations))
  h0_joe <- mclapply_progress(seq_len(num_simulations),
                              function(i) compute_stats(pobs(cbind(runif(n), runif(n)))),
                              mc.cores = ncores, label = "critical values")
...
for (n in n_grid) {
  ...
  plot(sub$theta, sub$Power_IECT, type = "b", pch = 16, col = "steelblue",
       ylim = c(0, 1), xlab = expression(theta), ylab = "Power",
       main = sprintf("Joe copula  (n = %d)", n), las = 1, cex.main = 1.9, lwd = 2)
  lines(sub$theta, sub$Power_ECT, type = "b", pch = 17, col = "darkorange", lwd = 2)
  abline(h = 0.05, lty = 2, col = "gray60")
  legend("topleft", legend = c("IECT", "ECT"),
         col = c("steelblue", "darkorange"), pch = c(16, 17), lty = 1, bty = "n", lwd = 2, cex = 1.3)
}


##============================================================
## Null hypothesis p-value histograms for IECT and ECT
##============================================================
...
for (n in n_grid) {
  ...
  ## Step 1: null reference distribution (num_simulations replicates)
  ...
  ## Step 2: separate H0 test statistics (num_power replicates)
  ...
  ## Step 3: compute p-values of test set against null reference
  ...
  hist(pval_IECT, breaks = 20, freq = FALSE,
       ...
       xlab = "p-value", col = "steelblue", border = "white", las = 1, cex.main = 1.9)
  abline(h = 1, lty = 2, col = "red", lwd = 2)

  hist(pval_ECT, breaks = 20, freq = FALSE,
       main = bquote("ECT — " * H[0] * " p-values  (n =" ~ .(n) * ")"),
       xlab = "p-value", col = "darkorange", border = "white", las = 1,cex.main = 1.9)
  abline(h = 1, lty = 2, col = "red", lwd = 2)
}


##============================================================
## Real Data Application: Uranium Exploration Data (Li vs Ti)
## Testing for independence using IECT and ECT
##============================================================
...
## --- Empirical tail dependence profile ---
...
## ============================================================
## Figure 1: Exploratory scatter plots (2 x 2)
## ============================================================
...
plot(U[, 1], U[, 2], pch = 16, cex = 0.4, col = "steelblue",
     ...
     main = sprintf("(b) Pseudo observations (tau = %.3f)", tau_hat), cex.main = 1.9)
...
plot(q_grid, lambda_U, type = "b", pch = 16, col = "red", lwd = 2,
...
legend("topright", legend = c(expression(hat(lambda)[U](q)),
                               expression(hat(lambda)[L](1-q))),
       col = c("red", "blue"), pch = c(16, 17), lty = 1, bty = "n", lwd = 2, cex = 1.7)
...
## ============================================================
## Figure 2: Null distribution histograms with observed values
## ============================================================

par(mfrow = c(1, 1), mar = c(5, 5, 3, 1), cex.lab = 1.7, cex.axis = 1.7, mgp = c(3.3, 0.8, 0))

## --- IECT ---
h1 <- hist(null_mat[, "Rnbar"], breaks = 40, plot = FALSE)
hist(null_mat[, "Rnbar"], breaks = 40, freq = FALSE, col = "steelblue", border = "white",
     main = "IECT null distribution", xlab = expression(bar(R)[n]), las = 1, cex.main = 1.9,
...
hist(null_mat[, "Bn"], breaks = 40, freq = FALSE, col = "darkorange", border = "white",
     main = "ECT null distribution", xlab = expression(R[n]), las = 1, cex.main = 1.9,
...
## ============================================================
## Figure 3: Copula fit comparison (for context)
## ============================================================
...
fits <- list(Joe = fit_joe, Gumbel = fit_gumbel, Frank = fit_frank, Normal = fit_normal)
for (nm in names(fits)) {
  cat(sprintf("  %-8s  param = %6.3f   logLik = %8.2f   AIC = %8.2f  BIC = %8.2f\n",
              nm, coef(fits[[nm]])[1], logLik(fits[[nm]]), AIC(fits[[nm]]), BIC(fits[[nm]])))
}
